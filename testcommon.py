...
if platform.system() == "Windows":
    dataspath = r'E:\python_work\backtrader\datas'
elif platform.system() == "Linux":
    dataspath = "../../datas"
else:
    dataspath = "../../datas"  # 默认使用 macOS 或其他 Unix-like 系统的路径
datafiles = [
    '2006-day-001.txt',
    '2006-week-001.txt',
]
...
def getdata(index, fromdate=FROMDATE, todate=TODATE):
    ...
    datapath = os.path.join(dataspath, datafiles[index])
    data = DATAFEED(
        dataname=datapath,
        fromdate=fromdate,
        todate=todate)
        ...
        for ronce in runonces:
            for exbar in exbars:
                ...
                if kwargs.get('main', False):
                    mylogger.info('prload {} / ronce {} exbar {}'.format(
                        prload, ronce, exbar))
...
class TestStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if self.p.main:
            ...
            mylogger.info('%s - %d - %f' % (dtstr, len(self), self.ind[0]))
            pstr = ', '.join(str(x) for x in
                             [self.data.open[0], self.data.high[0],
                              self.data.low[0], self.data.close[0]])
            mylogger.info('%s - %d, %s' % (dtstr, len(self), pstr))
    ...
    def stop(self):
        ...
        if self.p.main:
            mylogger.info('----------------------------------------')
            mylogger.info('len ind %d == %d len self' % (l, len(self)))
            mylogger.info('minperiod %d' % self.chkmin)
            mylogger.info('self.p.chknext %d nextcalls %d'
                  % (self.p.chknext, self.nextcalls))

            mylogger.info(f'chkpts are {chkpts}')
            for chkpt in chkpts:
                dtstr = self.data.datetime.date(chkpt).strftime('%Y-%m-%d')
                mylogger.info('chkpt %d -> %s' % (chkpt, dtstr))
            ...
            for lidx in range(self.ind.size()):
                ...
                mylogger.info(outtxt)

            mylogger.info('vs expected')

            for chkval in self.p.chkvals:
                mylogger.info(chkval)
