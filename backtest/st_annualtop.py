...
def calculate_metrics(df_year, stock_name): 
    ...
    if len(closes) < 2:
        return StockResult.get_dict(Code=stock_name)
...
def process_stock(stock_name, stockdata_path, start_date=''):
    """
    单个股票的处理函数，供进程池调用
    """   
    try:
...
def run_strategy(target_year='', batch_size=1000):
    write_buffer = []
    all_results_buffer = []
    ...
    df_stocklist = pd.read_csv(STOCKLIST_FILE, usecols=[0,2], skiprows=1, header=None, dtype={0: str}) #read_csv返回的DF数据格式
    stock_list = df_stocklist[0].tolist()  # 转为 list 格式
    ...
    if all_results_buffer:
        # 按涨幅降序排列
        mapping_dict = dict(zip(df_stocklist[0], df_stocklist[2]))
        df_all = pd.DataFrame(all_results_buffer)
        df_all['Name'] = df_all['Code'].map(mapping_dict)
